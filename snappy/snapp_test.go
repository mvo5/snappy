...
func (s *SnapTestSuite) TestLocalSnapSimple(c *C) {
	...
	st, err := os.Stat(snap.basedir.Dir())
	c.Assert(err, IsNil)
	c.Assert(snap.Date(), Equals, st.ModTime())

	c.Assert(snap.basedir.Dir(), Equals, filepath.Join(s.tempdir, "apps", helloAppComposedName, "1.10"))
...
`)
	...
	part := &SnapPart{m: m, origin: testOrigin, basedir: local.New(d1)}
	c.Assert(part.RefreshDependentsSecurity(&SnapPart{basedir: local.New(d2)}, pb), IsNil)
...
func (s *SnapTestSuite) TestRequestSecurityPolicyUpdateService(c *C) {
	...
	part := &SnapPart{m: &packageYaml{Name: "part", ServiceYamls: []ServiceYaml{svc}, Version: "42"}, origin: testOrigin, basedir: local.New(filepath.Join(dirs.SnapAppsDir, "part."+testOrigin, "42"))}
...
func (s *SnapTestSuite) TestRequestSecurityPolicyUpdateBinary(c *C) {
	...
	part := &SnapPart{m: &packageYaml{Name: "part", Binaries: []Binary{bin}, Version: "42"}, origin: testOrigin, basedir: local.New(filepath.Join(dirs.SnapAppsDir, "part."+testOrigin, "42"))}
	err := part.RequestSecurityPolicyUpdate(nil, map[string]bool{"foo": true})
