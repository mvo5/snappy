...
func (inst *snapInstruction) install() interface{} {
	...
	err := snapstate.Install(chg, inst.pkg, inst.Channel)
	state.Unlock()
...
func (inst *snapInstruction) remove() interface{} {
	...
	name, _ := snappy.SplitDeveloper(inst.pkg)
	err := snapstate.Remove(chg, name)
	state.Unlock()
