...
func getConns(st *state.State) (conns map[string]connState, err error) {
	...
	for id, cstate := range conns {
		...
		cref.PlugRef.Snap = snap.IfaceRemapSnapFromState(cref.PlugRef.Snap)
		cref.SlotRef.Snap = snap.IfaceRemapSnapFromState(cref.SlotRef.Snap)
...
func setConns(st *state.State, conns map[string]connState) {
	...
	for id, cstate := range conns {
		...
		cref.PlugRef.Snap = snap.IfaceRemapSnapToState(cref.PlugRef.Snap)
		cref.SlotRef.Snap = snap.IfaceRemapSnapToState(cref.SlotRef.Snap)
		remapped[cref.ID()] = cstate
	}
...
func resolveSnapIDToName(st *state.State, snapID string) (name string, err error) {
	if snapID == "system" {
		...
		return snap.IfaceRemapSnapFromRequest(snapID), nil
	}
