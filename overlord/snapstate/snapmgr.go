...
func (m *SnapManager) doInstallSnap(t *state.Task) error {
	...
	t.State().Unlock()
	println("doInstall", name, channel)
...
func (m *SnapManager) doRemoveSnap(t *state.Task) error {
	...
	t.Get("name", &name)
	t.State().Unlock()
	println("doRemove", name)

	err := snappy.Remove(name, 0, &progress.NullProgress{})
	fmt.Println(err)
	return err
}
