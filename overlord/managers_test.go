...
`
	...
	ts, err := snapstate.Install(nil, st, "foo", "stable", snap.R(0), 0, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg := st.NewChange("install-snap", "...")
	chg.AddAll(ts)
...
func (ms *mgrsSuite) TestHappyRemoteInstallAndUpdateManyWithEpochBump(c *C) {
	...
	affected, tasksets, err := snapstate.InstallMany(nil, st, snapNames, 0)
	c.Assert(err, IsNil)
...
`
	...
	ts, err := snapstate.Install(nil, st, "foo", "stable", snap.R(0), 0, snapstate.Flags{Unaliased: true})
	c.Assert(err, IsNil)
	chg := st.NewChange("install-snap", "...")
	chg.AddAll(ts)
...
`
	...
	ts, err = snapstate.Install(nil, st, "bar", "stable", snap.R(0), 0, snapstate.Flags{})
	c.Assert(err, IsNil)
	chg = st.NewChange("install-snap", "...")
	chg.AddAll(ts)
