...
func handleCertConfiguration(tr config.Conf) error {
	for _, name := range tr.Changes() {
		...
		certPath := filepath.Join(dirs.SnapExtraSslCertsDir, optionName+".pem")
		switch cert {
		case "":
			// remove
			os.Remove(certPath)
		default:
			os.MkdirAll(dirs.SnapExtraSslCertsDir, 0755)
	...
	for _, name := range tr.Changes() {
		...
		if cert != "" {
			...
			if block == nil || block.Type != "CERTIFICATE" || len(rest) > 0 {
				return fmt.Errorf("cannot decode pem certificate %v", optionName)
			}
