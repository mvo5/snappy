...
func (s *mountedfilesystemTestSuite) TestMountedUpdaterTrivialValidation(c *C) {
	...
	rw, err := gadget.NewMountedFilesystemUpdater(psNoFs, s.backup, lookupFail)
	c.Assert(err, ErrorMatches, "structure #0 has no filesystem")
	c.Assert(rw, IsNil)
	...
	rw, err = gadget.NewMountedFilesystemUpdater(ps, "", lookupFail)
	c.Assert(err, ErrorMatches, `internal error: backup directory must not be unset`)
	c.Assert(rw, IsNil)

	rw, err = gadget.NewMountedFilesystemUpdater(ps, s.backup, nil)
	c.Assert(err, ErrorMatches, `internal error: mount lookup helper must be provided`)
	c.Assert(rw, IsNil)

	rw, err = gadget.NewMountedFilesystemUpdater(nil, s.backup, lookupFail)
	c.Assert(err, ErrorMatches, `internal error: \*LaidOutStructure.*`)
	c.Assert(rw, IsNil)
	...
	for _, tc := range []struct {
		...
		rw, err := gadget.NewMountedFilesystemUpdater(testPs, s.backup, lookupOk)
		c.Assert(err, IsNil)
		c.Assert(rw, NotNil)
