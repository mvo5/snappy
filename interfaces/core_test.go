...
func (s *CoreSuite) TestPlugRef(c *C) {
	plug := &Plug{PlugInfo: &snap.PlugInfo{snap.PlugSlotData{Snap: &snap.Info{SuggestedName: "consumer"}, Name: "plug"}}}
	ref := plug.Ref()
...
func (s *CoreSuite) TestSlotRef(c *C) {
	slot := &Slot{SlotInfo: &snap.SlotInfo{snap.PlugSlotData{Snap: &snap.Info{SuggestedName: "producer"}, Name: "slot"}}}
	ref := slot.Ref()
