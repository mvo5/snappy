...
func (s *backendSuite) SetUpTest(c *C) {
	...
	err := os.MkdirAll(seccomp.SnapSeccompDir(), 0700)
	c.Assert(err, IsNil)
}
...
func (s *backendSuite) TestInstallingSnapWritesProfiles(c *C) {
	...
	profile := filepath.Join(seccomp.SnapSeccompDir(), "snap.samba.smbd")
...
func (s *backendSuite) TestInstallingSnapWritesHookProfiles(c *C) {
	...
	profile := filepath.Join(seccomp.SnapSeccompDir(), "snap.foo.hook.configure")
...
func (s *backendSuite) TestUpdatingSnapToOneWithMoreApps(c *C) {
	for _, opts := range testedConfinementOpts {
		...
		profile := filepath.Join(seccomp.SnapSeccompDir(), "snap.samba.nmbd")
		_, err := os.Stat(profile)
...
func (s *backendSuite) TestUpdatingSnapToOneWithHooks(c *C) {
	for _, opts := range testedConfinementOpts {
		...
		profile := filepath.Join(seccomp.SnapSeccompDir(), "snap.samba.hook.configure")
...
func (s *backendSuite) TestCombineSnippetsOrdering(c *C) {
	...
	profile := filepath.Join(seccomp.SnapSeccompDir(), "snap.foo.foo")
	data, err := ioutil.ReadFile(profile)
	c.Assert(err, IsNil)
